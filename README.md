...
> **Что:** платформа анализа футбольных тренировок — носимый трекер ESP32-S3 (GPS + IMU) шлёт телеметрию, бэкенд считает метрики, React-дашборд обновляется в реальном времени.
> **Стек:** Go (chi, pgx/v5, goose, gorilla/websocket, JWT), React 18 (Vite, Recharts), PostgreSQL 15, Redis 7, Docker Compose, GitHub Actions.
> **Результат:** фильтр Калмана для слияния GPS/IMU, метрики (спринты, ускорения, Player Load, зоны интенсивности, высота прыжка), тепловые карты 10×10 м, live-поток по WebSocket.
